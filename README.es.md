...
descarga las dependencias nativas del motor. Un paso `postinstall` marca como
ejecutables los binarios auxiliares precompilados de `node-pty`, y en Windows
retorna de inmediato.

### Detener lo que ya esté en ejecución

OpenFlow usa dos puertos: **4096** para el motor y **5174** para el lienzo. Si
una ejecución anterior los sigue ocupando, arrancar el motor a mano falla con
`Error: Unexpected error` / `ServeError`: es un puerto ocupado, no una
instalación rota.

`bun openflow.ts` se encarga de esto: reutiliza un puerto que ya está sirviendo
y libera el que dejó ocupado una ejecución muerta. Mata los procesos antiguos tú
mismo solo cuando quieras un motor realmente nuevo — por ejemplo tras editar
`opencode.json`, ya que el motor cachea la configuración del proyecto al
arrancar y no vuelve a leerla.
...
Ambos comandos son seguros cuando no hay nada escuchando: simplemente no
encuentran ningún proceso. Matan cualquier proceso en esos puertos, así que
comprueba antes si ejecutas algo más ahí:
...
**Si el motor falla con `database is locked`,** ya hay otro motor de opencode
ejecutándose en otro sitio: una segunda copia de OpenFlow, o uno que dejó un
lanzador anterior. Todos los motores comparten una única base de datos en tu
directorio de datos de opencode, así que el segundo en arrancar no puede abrirla.
Liberar los puertos no sirve aquí, porque ese motor puede no estar ocupándolos.
Detenlo por lo que está ejecutando:
...
Después vuelve a iniciar OpenFlow. Si ocurre siempre, lo más probable es que
tengas OpenFlow clonado en dos sitios y se arranquen los dos: quédate con uno.

### Ejecutar

Un solo comando arranca ambos procesos, igual en todas las plataformas:

```bash
bun openflow.ts
```

Arranca el motor, espera hasta que responda y luego abre el lienzo en
http://localhost:5174; Ctrl+C detiene ambos. Un puerto que dejó ocupado una
...
Las flags son opcionales, y las tres superficies se resuelven en el mismo plan:

| PowerShell | shell | entorno | qué hace |
|---|---|---|---|
| `-Project <dir>` | `-p`, `--project <dir>` | `OPENFLOW_PROJECT` | el repositorio que los agentes leen y escriben — **editan archivos reales** |
| `-ServerPort <n>` | `-s`, `--server-port <n>` | `OPENCODE_SERVER_URL` | puerto del motor, 4096 por defecto |
| `-Built` | `-b`, `--built` | `OPENFLOW_BUILT=1` | compilar y servir el bundle estático en lugar de ejecutar vite |
| `-Manage` | `-m`, `--manage` | `FLOW_MANAGE_SERVER=1` | dejar que el lienzo sea dueño del motor, lo que hace funcionar su botón de reinicio |
| `-Help` | `-h`, `--help` | — | imprimir la lista de flags |
| — | — | `OPENFLOW_DRY_RUN=1` | imprimir el plan resuelto y no arrancar nada |
