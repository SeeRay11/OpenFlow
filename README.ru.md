...
Два шима оборачивают тот же самый файл — для тех, кому привычнее лаунчер своей
платформы. Собственной логики у них нет: они лишь превращают флаги в переменные
окружения, которые `openflow.ts` и так читает.
...
**В Windows шим может отказаться запускаться:** PowerShell по умолчанию не выполняет
...
Чтобы всё же использовать шим, разрешите локальные скрипты для своей учётной записи
