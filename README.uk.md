...
### Зупинити те, що вже працює

OpenFlow займає два порти: **4096** для рушія та **5174** для полотна. Якщо їх
досі тримає попередній запуск, ручний старт рушія завершується `Error:
Unexpected error` / `ServeError` — це зайнятий порт, а не зламане встановлення.

`bun openflow.ts` робить це сам: він повторно використовує порт, який уже
відповідає, і звільняє той, що лишився зайнятим після мертвого запуску. Убивати
старі процеси вручну варто лише тоді, коли потрібен справді свіжий рушій —
наприклад, після зміни `opencode.json`, бо рушій кешує конфігурацію проєкту під
час старту й більше її не перечитує.
...
Обидві команди безпечні, коли ніщо не слухає: вони просто не знайдуть процесу.
Вони вбивають будь-який процес на цих портах, тож спершу перевірте, чи не
запущено там щось інше:
