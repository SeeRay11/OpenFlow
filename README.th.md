...
มันจะเริ่มเอนจิน รอจนกว่าเอนจินจะตอบกลับ แล้วเปิดผืนผ้าใบที่ http://localhost:5174
ส่วน Ctrl+C จะหยุดทั้งสอง พอร์ตที่การรันซึ่งตายไปแล้วยังจองค้างไว้จะถูกปล่อยก่อน และพอร์ตที่
ให้บริการอยู่แล้วจะถูกนำมาใช้ซ้ำแทนที่จะเริ่มซ้ำอีกครั้ง

มีสคริปต์บาง ๆ สองตัวที่ห่อไฟล์เดียวกันนี้ไว้ สำหรับคนที่ถนัดตัวเรียกใช้ของแพลตฟอร์มตัวเอง
ทั้งสองไม่มีตรรกะของตัวเอง — เพียงแปลงแฟล็กให้เป็นตัวแปรสภาพแวดล้อมที่ `openflow.ts` อ่านอยู่แล้ว
...
**บน Windows สคริปต์ตัวนี้อาจปฏิเสธที่จะเริ่มทำงาน:** โดยค่าเริ่มต้น PowerShell จะไม่รันสคริปต์
ในเครื่องที่ไม่ได้เซ็นชื่อ ดังนั้น `.\openflow.ps1` อาจล้มเหลวด้วยข้อความ *"openflow.ps1
cannot be loaded because running scripts is disabled on this system"* และรีโปที่ดาวน์โหลด
มาเป็น ZIP แทนการ clone ยังถูกทำเครื่องหมายว่ามาจากอินเทอร์เน็ต ซึ่งบล็อกมันอีกทางหนึ่งด้วย
`bun openflow.ts` ไม่ติดข้อจำกัดทั้งสองข้อนี้ และเป็นทางที่สั้นที่สุดในการข้ามทั้งคู่ หากยัง
ต้องการใช้สคริปต์ตัวนี้ ให้อนุญาตสคริปต์ในเครื่องสำหรับบัญชีของคุณเอง และปลดบล็อกไฟล์หากมันมาจาก ZIP:
...
แฟล็กเป็นตัวเลือกเสริม และทั้งสามช่องทางให้ผลลัพธ์เป็นแผนเดียวกัน:

| PowerShell | shell | ตัวแปรสภาพแวดล้อม | ทำอะไร |
|---|---|---|---|
| `-Project <dir>` | `-p`, `--project <dir>` | `OPENFLOW_PROJECT` | รีโปที่เอเจนต์อ่านและเขียน — **พวกมันแก้ไขไฟล์จริง** |
| `-ServerPort <n>` | `-s`, `--server-port <n>` | `OPENCODE_SERVER_URL` | พอร์ตของเอนจิน ค่าเริ่มต้น 4096 |
| `-Built` | `-b`, `--built` | `OPENFLOW_BUILT=1` | build แล้วเสิร์ฟบันเดิลแบบสแตติกแทนการรัน vite |
| `-Manage` | `-m`, `--manage` | `FLOW_MANAGE_SERVER=1` | ให้ผืนผ้าใบเป็นเจ้าของเอนจิน ซึ่งทำให้ปุ่มรีสตาร์ตของมันใช้งานได้ |
| `-Help` | `-h`, `--help` | — | พิมพ์รายการแฟล็ก |
| — | — | `OPENFLOW_DRY_RUN=1` | พิมพ์แผนที่ได้ออกมาโดยไม่เริ่มอะไรเลย |
